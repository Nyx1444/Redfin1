...
const FilterViewBar = ({ setViewOption }) => {
    ...
    const shouldShowFilter = (filterName) => {
        ...
        if (windowSize.width <= 900 && filterName === 'BedsBathsFilter') return false;
        if (windowSize.width <= 1000 && filterName === 'HomeTypeFilter') return false;
        return true;
    };
    ...
    return (
        <div style={{
            ...
            <div style={{ ...styles.filterGroup, alignSelf: isSize500 ? 'flex-start' : 'center' }}>
                {shouldShowFilter('TypeFilter') && (
                    <StatusFilter isOpen={activeDropdown === 'TypeFilter'} onToggle={() => toggleDropdown('TypeFilter')} />
                )}
                {shouldShowFilter('PriceFilter') && (
                    <PriceFilter isOpen={activeDropdown === 'PriceFilter'} onToggle={() => toggleDropdown('PriceFilter')} />
                )}
                {shouldShowFilter('BedsBathsFilter') && (
                    <BedsBathsFilter isOpen={activeDropdown === 'BedsBathsFilter'} onToggle={() => toggleDropdown('BedsBathsFilter')} />
                )}
                {shouldShowFilter('HomeTypeFilter') && (
                    <HomeTypeFilter isOpen={activeDropdown === 'HomeTypeFilter'} onToggle={() => toggleDropdown('HomeTypeFilter')} />
                )}
                <button
                    style={{
                        ...styles.button,
