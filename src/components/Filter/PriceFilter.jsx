...
const PriceFilter = () => {
    ...
    const getPercent = (value) => ((value - minValue) * 100) / (maxValue - minValue);
            ...
            {isOpen && (
                <div style={styles.dropdown}>
                    <div style={styles.sliderContainer} ref={sliderRef}>
                        ...
                        <div
                            style={{ ...styles.sliderThumb, left: `${getPercent(range[0])}%` }}
                            onMouseDown={(e) => handleThumbMove(0, e)}
                        ></div>
                        <div
                            style={{ ...styles.sliderThumb, left: `${getPercent(range[1])}%` }}
                            onMouseDown={(e) => handleThumbMove(1, e)}
                        ></div>
                    </div>
                    <div style={styles.inputContainer}>
                        <input type="number" value={range[0]} style={styles.input} readOnly />
                        <span>-</span>
                        <input type="number" value={range[1]} style={styles.input} readOnly />
                    </div>
                    <div style={styles.buttonGroup}>
                        <button style={styles.resetButton} onClick={() => setRange([50000, 10000000])}>Reset</button>
                        <button style={styles.doneButton} onClick={() => setIsOpen(false)}>Done</button>
                    </div>
                </div>
            )}
        </div>
