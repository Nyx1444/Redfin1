...
const ViewOption = ({ onSelect }) => {
  const [selected, setSelected] = useState(localStorage.getItem('viewOption') || 'List'); // ดึงค่าจาก localStorage ถ้ามี
      ...
      {['List', 'Split', 'Map'].map((option) => (
        <button
          ...
          style={{
            ...styles.button,
            backgroundColor:
              selected === option ? '#00828C' :
                hoveredButton === option ? '#DFF6F5' : 'transparent',
            color:
              selected === option ? '#fff' :
                hoveredButton === option ? '#00828C' : '#000',
